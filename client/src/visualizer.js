      ...
      var setData = function(_sourceData) {
        ...
        $.each(markedText, function(textNo, textPos) {
          ...
          if (startChunk == numChunks) {
            var chunk = data.chunks[startChunk];
            dispatcher.post('messages', [[['Wrong text offsets', 'error']]]);
            return;
          }
...
Util.profileStart('before render');
      ...
      var addedAnnotations = function(added, source){
          //Partialy redraw of added annotations, now only entities and no curlys, no check on size of entity name vs chunk size
          //won't give decent visualisation for most cases. 
          $.each(added, function(addedNo, add){
              if(add[0][0] === "T"){
                  //dispatcher.post('messages', [[['Added ', 'error']]]);
                  var chunk = $('#svg').find('tspan[data-id="'+add[1]+'"]');
                  var trans = $('#svg').find('g[transform="translate(0, '+chunk.attr('y')+')"]');
                  var span = trans.find('g[transform="translate('+chunk.attr('x')+', 0)"]');
                  g = document.createElementNS('http://www.w3.org/2000/svg', "g");
                  g.setAttribute("class", "span");
                  rect = document.createElementNS('http://www.w3.org/2000/svg', "rect");
                  rect.setAttribute("x", "0.5");
                  rect.setAttribute("y", "-28.5");
                  rect.setAttribute("width", chunk.width());
                  rect.setAttribute("height", "12");
                  rect.setAttribute("data-span-id", add[0]);
                  rect.setAttribute("class", 'span_'+add[3]);
                  var spanDesc = spanTypes[add[3]];
                  var bgColor = ((spanDesc && spanDesc.bgColor) ||
                                 (spanTypes.SPAN_DEFAULT && spanTypes.SPAN_DEFAULT.bgColor) ||
                                 '#ffffff');
                  var borderColor = ((spanDesc && spanDesc.borderColor) ||
                  ...
                  text.setAttribute("x", 0.5+(chunk.width()/2));
                  text.setAttribute("y", "-19");
                  text.setAttribute("fill", "black");
                  span.append(g);
                  span.find('g[class="span"]').append(rect);
                  span.find('g[class="span"]').append(text);
                  $(text).append(add[3]);
                  var hl = document.createElementNS('http://www.w3.org/2000/svg', "rect");
                  var lightBgColor = Util.adjustColorLightness(bgColor, 0.8);
                  hl.setAttribute("x", chunk.attr('x'));
                  hl.setAttribute("y", (chunk.attr('y')-11));
                  hl.setAttribute("width", chunk.width());
                  hl.setAttribute("height", "14");
                  hl.setAttribute("rx", "3");
                  hl.setAttribute("ry", "3");
                  hl.setAttribute("fill", lightBgColor);
                  $('#svg').find('g[class="highlight"]').append(hl);
              }
          });
      };
	//visualLayers added by Sander Naert
      dispatcher.
          ...
          on('visualLayers', setVisualLayers).
          on('addedAnnotations', addedAnnotations).
