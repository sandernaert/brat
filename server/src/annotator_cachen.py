...
from annotation import (OnelineCommentAnnotation, TEXT_FILE_SUFFIX,
        ...
        NormalizationAnnotation, SpanOffsetOverlapError, DISCONT_SEP, SimpleAnnotations,AnnotationNotFoundError)
    ...
    def json_response(self, response=None):
        ...
        for a in self.__added:
            try:
                ...
                response['edited'].append(a.id)
                response['added'].append([a.id,a.spans[0][0],a.tail.strip(),a.type])
...
def _json_from_ann(ann_obj):
    ...
    #Changed by sander naert so it works with the caching
    from document import (_enrich_json_with_data, _enrich_json_with_base,
        _enrich_json_with_text)
    from ApplicationScope import getAnnObject,filter_layers,filter_folia
    ...
    try:
        #select which annotations should be visualised, remove others
        import os
        docdir = os.path.dirname(ann_obj._document)
        filter_layers(j_dic,docdir)
        filt_folia = filter_folia(ann_obj)
        #add validation info
        #Standard for validation is ON
        try:
...
def _create_span(collection, document, offsets, _type, attributes=None,
    ...
    _document = path_join(real_dir, document)

    projectconf = ProjectConfiguration(real_dir)

    txt_file_path = _document + '.' + TEXT_FILE_SUFFIX

    working_directory = path_split(document)[0]
    
    with getAnnObject (collection, document) as ann_obj:
        ...
        ann_obj.issues = update_issues(projectconf,mods,ann_obj)
...
def delete_span(collection, document, id):
    ...
    with getAnnObject(collection, document) as ann_obj:
        ...
        try:
            ann = ann_obj.get_ann_by_id(id)
        except AnnotationNotFoundError, e:
...
def set_status(directory, document, status=None):
    ...
    with getAnnObject(directory, document) as ann_obj:
...
def get_status(directory, document):
    real_dir = real_directory(directory)
    with TextAnnotations(path_join(real_dir, document),
...
from annotation import (TextAnnotations, TEXT_FILE_SUFFIX,
        ...
        open_textfile,
        BIONLP_ST_2013_COMPATIBILITY,TextBoundAnnotation, AttributeAnnotation, BinaryRelationAnnotation, EventAnnotation, EquivAnnotation,NormalizationAnnotation, OnelineCommentAnnotation)
...
def enrich_json_with_data(j_dic, ann_obj):
    ...
    for ann in ann_obj:
        ...
        elif isinstance(ann,TextBoundAnnotation):
            tb_ann = ann
        ...
        elif isinstance(ann,EquivAnnotation):
            eq_ann = ann
            j_dic['equivs'].append(
                    (['*', eq_ann.type]
        ...
        elif isinstance(ann,OnelineCommentAnnotation):
            com_ann = ann
...
def update_issues(proj, mods, ann):
    issues = []
    try:
        # Validation
        import simplejson as json
        ...
        return issues
    if val:
        ids = [i.id for i in mods.get_added() if isinstance(i, TextBoundAnnotation)]
        del_for_ids = []
        for i in mods.get_added():
            if isinstance(i, TextBoundAnnotation):
                ids.append(i.id)
            if isinstance(i, BinaryRelationAnnotation):
                ids.append(i.arg1)
            if isinstance(i, EquivAnnotation):
                ids.append(i.entities[0])
            if isinstance(i, EventAnnotation):
                ids.append(i.trigger)
            if isinstance(i, AttributeAnnotation):
                ids.append(i.target)
        for j in mods.get_changed():
            #j is a set of text and the annotation
            i = j[1]
            if isinstance(i, TextBoundAnnotation):
                ids.append(i.id)
            if isinstance(i, BinaryRelationAnnotation):
            ...
            if isinstance(i, AttributeAnnotation):
                ids.append(i.target)
        for i in mods.get_deleted():
            if isinstance(i, TextBoundAnnotation):
                del_for_ids.append(i.id)
            if isinstance(i, EventAnnotation):
                del_for_ids.append(i.trigger)
                for j in i.args:
                    ids.append(j[1])
            if isinstance(i, BinaryRelationAnnotation):
                ids.append(i.arg1)
            ...
            if isinstance(i, AttributeAnnotation):
                ids.append(i.target)    
        #need to remove ids of textboundannotations that are deleted
        #this is needed when an textboundannotations with attributes is deleted
        ids[:] = [i for i in ids if i not in del_for_ids ]
        from validation_rule import ValidationRules
        if ids:
            vrules = ValidationRules(proj)
            response = vrules.validate(ann,ids)
            issues = [ issue for issue in ann.issues if not issue.ann_id in response[1] ]
            issues += response[0]
        if del_for_ids:
            issues = [ issue for issue in ann.issues if not issue.ann_id in del_for_ids ]
    return issues

#Added by sander naert for testing
if __name__ == '__main__':
    
    create_span("/brat_vb/sentiment/","sporza",'[[12,25]]', "PolarExpr",'{"Type": "2"}')
