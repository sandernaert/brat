...
__optional_annotation_sections = (RULE_SECTION,TEXT_SECTION)
...
def __parse_configs(configstr, source, expected_sections, optional_sections):
    ...
    for ln, l in enumerate(configstr.split("\n")):
        ...
        if m:
            ...
            #sander naert: added second part so no warnings if optional section is present
            if section not in expected_sections and section not in optional_sections:
...
class ProjectConfiguration(object):
    ...
    #added by sander Naert
    def get_rules_type_hierarchy(self):
        return get_rules_type_hierarchy(self.directory)
    def get_text_type_hierarchy(self):
        return get_text_type_hierarchy(self.directory)
