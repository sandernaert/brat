...
def _json_from_ann(ann_obj):
    ...
    #~ j_dic = getAnnObject("/brat_vb/sentiment","test")
...
if __name__ == '__main__':
    from annotation import SimpleAnnotations
    import time
    sec = int(round(time.time() * 1000))
    print sec
    print "CREATE"
    create_span("/brat_vb/sentiment/","test",'[[13,25]]', "PolarExpr",'{"Type": "3"}')
    print "DELETE"
    delete_span("/brat_vb/sentiment/","test", "T251")
    sec2 = int(round(time.time() * 1000))
    print "total time "+str(sec2-sec)+" "+ str(sec2)
